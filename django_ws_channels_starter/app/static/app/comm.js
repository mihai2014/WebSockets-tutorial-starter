window.addEventListener("DOMContentLoaded", () => {    
    ...
    // const testSocket = new WebSocket('ws://echo/');
    // testSocket = new WebSocket('ws://localhost:8000/ws/echo/');

    const clock_div = document.querySelector('#clock');

    // get time
    const time_websocket = new WebSocket(
        'ws://'
    ...
    //send echo message
    const echo_websocket = new WebSocket(
        'ws://'
        + window.location.host
        + '/ws/echo/'
    );
